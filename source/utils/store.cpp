...
Store::Store(nlohmann::json &JS, std::string JSONName) {
	...
	if (access("sdmc:/3ds/Ghost-Eshop/updates.json", F_OK) != 0) {
		// We'd create the file here.
		FILE *file = fopen("sdmc:/3ds/Ghost-Eshop/updates.json", "w");
		...
		FILE *file2 = fopen("sdmc:/3ds/Ghost-Eshop/updates.json", "r");
		this->updateJSON = nlohmann::json::parse(file2, nullptr, false);
		fclose(file2);
	} else {
		FILE *file = fopen("sdmc:/3ds/Ghost-Eshop/updates.json", "r");
